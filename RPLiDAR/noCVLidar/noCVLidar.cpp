...
float minLineLength = 155;
float maxLineLength = 165;
...
//Translate rplidar polar points into a standard cartesian grid
...
/* 
Inputs the polar coord of a point and sets distance to 0 if the angle is outside of target range. To be called before 
pointArray is filled.
*/
void filterAngle(float& theta, float& distance) {
...
/*
Finds all points that are about the target line length away from the inputted point
	inputArray - array of points (cartesian)
	inputPointPos - the array address of the origin point
	outputList - A list of array addresses (relating to inputArray) of all points that form a line with the origin point
				that is within the accceptable length
*/
void findDistantPoints(Point[]& inputArray[], int inputPointPos, List<Point>& outputList){
    size_t   count = _countof(inoutArray);
